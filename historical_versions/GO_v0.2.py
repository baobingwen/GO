...
class GO:
    ...
    def __init__(self, board_dimension = 19, cell_edge_len = 40):
        ...
        # 图片资源
        self.image_blackStone = pygame.image.load('./images/blackStone.png')
        self.image_whiteStone = pygame.image.load('./images/whiteStone.png')
        ...
        for x in range(1, self.board_dimension + 1):
            for y in range(1, self.board_dimension + 1):
                if self.current_board[x][y] != '.':
                    ...
                    stone_rect = pygame.rect.Rect(center_x - 18,center_y - 18,36,36)
                    
                    # 放置单个棋子
                    if self.current_board[x][y] == 'B':
    ...
    # 后端落子
    def drop_stone(self, coordinate):
        # 处理棋子位置
        x, y = coordinate
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:  # 点击左键
                ...
                game.drop_stone(game.pos_to_coordinate(event.pos))
